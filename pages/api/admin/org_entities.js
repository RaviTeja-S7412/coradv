...
export default async function handler(req, res) {
    switch (req.body.action) {
        ...
        case "DELETE": {
            return deleteOrgentity(req, res);
        }
    }
}
...
async function getOrgentities(req,res){
    ...
    org_entities.aggregate([
        ...
        org_entities.find({$and: [{ is_deleted: 0 }], $or: 
            [ 
                { org_entity_id: { "$regex": search, "$options": "i"} },
                { org_entity_name: { "$regex": search, "$options": "i"} },
                { org_region: { "$regex": search, "$options": "i"} },
...
async function createOrgentity(req,res){
    ...
    if(req.body.action == "UPDATE"){
        ...
        data["modified_by"] = req.body.modified_by;
    }else{
...
async function deleteOrgentity(req, res){

    const id = req.body.entity_id;
    if(!id){
        return res.status(202).json({ status:400, message: "ID is Required." });
    }
    const org_entities = await db.collection("ca_org_entities"); 
    org_entities.updateOne({_id:new ObjectId(req.body.entity_id)}, {$set: {is_deleted:1}}, function (error, result) {
        console.log(result)
        
        if (result) {
            return res.status(200).json({ status:200, message: "Organization Entity Deleted Successfully" });
        }else{
            return res.status(202).json({ status:400, message: "Organization Entity Not Found" });
        }
