            ...
            {data.map((obj, index) => {
                     const { id,url, title,subTitle } = obj;
                          ...
                          <img  src={url}   />
                                    ...
                                    <div className='absolute backdrop-blur-sm backdrop-filter flex flex-col items-center  justify-center p-4 w-full ' key={id}>
                ...
                {Array.from({length: 2}).map((item, index) => (
                    ...
                    className={`${slideIndex === index + 1 ? `${classes.dot} ${classes.active}` : classes.dot}`}
                    > </div>                   
