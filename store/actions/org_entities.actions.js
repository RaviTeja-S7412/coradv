...
export const deleteOrgentity = (udata) => {
  return async (dispatch) => {
    dispatch({ type: orgentityConstants.DELETE_ORGENTITY_REQUEST })
    udata["action"] = "DELETE";
    const res = await axiosIntance(`/api/admin/org_entities`, udata, 'post')

    if (res.status === 200) {
      dispatch({
        type: orgentityConstants.DELETE_ORGENTITY_SUCCESS,
        payload: { message: res.message },
      })
    } else {
      dispatch({
        type: orgentityConstants.DELETE_ORGENTITY_FAILURE,
        payload: { message: res.message },
      })
    }
  }
